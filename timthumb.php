...
if(! defined('MAX_FILE_SIZE') )				define ('MAX_FILE_SIZE', 100485760);						// 10 Megs is 10485760. This is the max internal or external file size that we'll process.  
...
class timthumb {
	...
	protected function serveErrors(){
		...
		$html .= '</ul>';
		echo '<h1>A IMG Compressor error has occured</h1>The following error(s) occured:<br />' . $html . '<br />';
		echo '<br />Query String : ' . htmlentities( $_SERVER['QUERY_STRING'], ENT_QUOTES );
		echo '<br />IMG Compressor version : ' . VERSION . '</pre>';
	}
	protected function serveInternalImage(){
		...
		if($fileSize > MAX_FILE_SIZE){
			...
			$this->error("तुम्हे नहीं लगता, की इस इमेज की साइज तुम्हारी औकात से बाहर  है.");
			return false;
		}
		if($fileSize <= 0){
